...
public class GrandsonEugeneDialogScript : MonoBehaviour
{
    ...
    void CreateSpeach_Hello()
    {
        ...
        SpeachNode root = new SpeachNode(npc_name, "Привет, путник!");
        root.is_answering = true;

        SpeachNode bye_node_1 = new SpeachNode(npc_name, "Ещё увидимся!");
        bye_node_1.answer_text = "Мне пора идти.";
        root.AddNextNode(bye_node_1);

        SpeachNode help_node_1 = new SpeachNode(npc_name, "Вообще-то я не маленький, мне уже 10!");
    ...
    void CreateSpeach_TheLostGrandson_ask_for_help_1()
    {
        ...
        SpeachNode root = new SpeachNode(npc_name, "Дедушка запрещает мне разговаривать с незнакомцами..."); // -------------------------------+
        root.is_answering = true; //                                                                                                 |
        //                                                                                                                           |
        SpeachNode bye_node_1 = new SpeachNode(npc_name, "Да, а как вы узнали моё имя?"); // -------------------------------------+            |
        ...
        root.AddNextNode(help_node_1); //                                                                               |
        //                                                                                                              |
        SpeachNode new_node_2 = new SpeachNode(npc_name, "Так вы пришли мне помочь! Ура!"); // -----------------------------------|------------+
        ...
        //                                                                                                                           |
        SpeachNode new_node_3 = new SpeachNode(npc_name, "Пойдём!"); //                                                                        |
