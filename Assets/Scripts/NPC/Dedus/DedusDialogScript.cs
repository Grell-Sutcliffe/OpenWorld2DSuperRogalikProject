...
public class DedusDialogScript : MonoBehaviour
{
    ...
    void CreateSpeach_Hello()
    {
        ...
        SpeachNode root = new SpeachNode(npc_name, "Здравствуй, путешественник!");
        root.is_answering = true;

        SpeachNode bye_node_1 = new SpeachNode(npc_name, "Как жаль. До встречи.");
        bye_node_1.answer_text = "Мне пора идти.";
        root.AddNextNode(bye_node_1);

        SpeachNode help_node_1 = new SpeachNode(npc_name, "Хорошего дня!");
    ...
    void CreateSpeach_AskForSearchGrandson_1()
    {
        ...
        TheLostGrandson_ask_for_search_grandson_1.npc_name = npc_name;
        ...
        SpeachNode help_node_1 = new SpeachNode(npc_name, "У меня случилась страшная беда. Не поможешь старичку?");
        help_node_1.is_answering = true;
        help_node_1.answer_text = "Привет, дедуль!";
        root.AddNextNode(help_node_1);

        SpeachNode help_node_2 = new SpeachNode(npc_name, "Ой, спасибо тебе, добрый молодой человек!");
        help_node_2.answer_text = "Конечно, помогу.";
        help_node_1.AddNextNode(help_node_2);

        SpeachNode bye_node_2 = new SpeachNode(npc_name, "До встречи.");
        bye_node_2.answer_text = "Мне некогда. Давай в другой раз?";
        help_node_1.AddNextNode(bye_node_2);

        SpeachNode help_node_3 = new SpeachNode(npc_name, "Мой внук потерялся, нигде не могу найти его уже 2 дня. Пожалуйста, найди его.");
        help_node_3.is_answering = true;
        help_node_2.AddNextNode(help_node_3);

        SpeachNode bye_node_3 = new SpeachNode(npc_name, "Ну и ну тебя нафиг, паразит маленький.");
        bye_node_3.answer_text = "Ой, нет. Сам ищи.";
        help_node_3.AddNextNode(bye_node_3);

        SpeachNode help_node_4 = new SpeachNode(npc_name, "Спасибо! До встречи.");
    ...
    void CreateSpeach_AskForSearchGrandson_2()
    {
        ...
        TheLostGrandson_ask_for_search_grandson_2.npc_name = npc_name;
        TheLostGrandson_ask_for_search_grandson_2.quest_title = questsController.quest_TheLostGrandson;

        SpeachNode root = new SpeachNode(npc_name, "Очень переживаю за внука."); // -----------------------------------------------------+
        root.is_answering = true; //                                                                                           |
        //                                                                                                                     |
        SpeachNode answer_node_1 = new SpeachNode(npc_name, "Попытаюсь вспомнить..."); // -----------------------------------------------|-----+
        answer_node_1.answer_text = "Где ты видел его в последний раз?"; // <--------------------------------------------------|     |
        root.AddNextNode(answer_node_1); //                                                                                    |     |
        //                                                                                                                     |     |
        SpeachNode answer_node_2 = new SpeachNode(npc_name, "Хммм... Дай-ка подумать..."); // -----------------------------------+       |     |
        answer_node_2.answer_text = "Не знаешь, куда он мог бы пойти?"; // <-------------------------------------------|-------+     |
        root.AddNextNode(answer_node_2); //                                                                            |             |     
        //                                                                                                             |             |
        SpeachNode answer_node_11 = new SpeachNode(npc_name, "Кажется, он отправился гулять с Джеком."); // <--------------------|-------------+
        answer_node_11.is_answering = true; // ------------------------------------------------------------------------|-------+
        answer_node_1.AddNextNode(answer_node_11); //                                                                  |       |
        //                                                                                                             |       |
        SpeachNode answer_node_21 = new SpeachNode(npc_name, "Обычно они с Джеком ходят гулять на окраину деревни."); // <-------+       |     
        answer_node_21.is_answering = true; // --------------------------------------------------------------------------------+
        answer_node_2.AddNextNode(answer_node_21); //                                                                          |
        //                                                                                                                     |
        SpeachNode answer_node_11_21_1 = new SpeachNode(npc_name, "Это наш пёс. Он очень дружелюбный и умный."); //                      |
        ...
        //                                                                                                                     |
        SpeachNode answer_node_11_21_2 = new SpeachNode(npc_name, "Удачи в поисках, добрый путник!"); //                                 |
    ...
    void CreateSpeach_AskForSearchGrandson_3()
    {
        ...
        TheLostGrandson_ask_for_search_grandson_3.npc_name = npc_name;
        TheLostGrandson_ask_for_search_grandson_3.quest_title = questsController.quest_TheLostGrandson;

        SpeachNode root = new SpeachNode(npc_name, "Внучок!");    
        root.is_ending = true;
